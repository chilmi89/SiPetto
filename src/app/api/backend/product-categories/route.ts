...
export async function GET(req: Request) {
    try {
        ...
        const where = {
            ...(profile_id ? { OR: [{ profile_id }, { profile_id: null }] } : (
                scope === "global" ? { profile_id: null } :
                scope === "tenant" ? { NOT: { profile_id: null } } : {}
            )),
        ...
        return NextResponse.json({ error: "Gagal mengambil data kategori produk" }, { status: 500 });
    }
}
...
export async function POST(req: Request) {
    try {
        ...
        return NextResponse.json({ error: "Gagal menyimpan kategori produk" }, { status: 500 });
    }
}
...
export async function PATCH(req: Request) {
    try {
        ...
        if (!id || !name) {
            return NextResponse.json({ error: "ID dan Nama kategori wajib disertakan" }, { status: 400 });
        }
        ...
        return NextResponse.json({ error: "Gagal memperbarui kategori produk" }, { status: 500 });
    }
}
...
export async function DELETE(req: Request) {
    try {
        ...
        if (!id) return NextResponse.json({ error: "ID kategori produk tidak ditemukan" }, { status: 400 });
        ...
        return NextResponse.json({ error: "Gagal menghapus kategori produk" }, { status: 500 });
    }
}
