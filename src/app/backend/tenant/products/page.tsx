...
interface Product {
    id: string;
    profile_id: string;
    branch_id: string | null; // NULL = Produk Pusat, UUID = Produk Cabang
...
export default function TenantProductsPage() {
    ...
    const [uploadingImage, setUploadingImage] = useState(false); // State upload gambar
    ...
    const fetchUserAndData = async () => {
        try {
            ...
            if (userRes.ok) {
                ...
                // Cek permission kelola_produk
                if (userData.permissions && !userData.permissions.includes("kelola_produk")) {
                ...
                if (branchId) {
                    productsUrl = `/api/backend/products?branch_id=${branchId}`;
                }

                const [productsRes, categoriesRes] = await Promise.all([
                    fetch(productsUrl),
                    fetch(`/api/backend/product-categories?profile_id=${profileId}&limit=100`)
                ]);
    ...
    // Handle upload gambar ke Supabase Storage via API
    ...
    const handleEditClick = (product: Product) => {
        ...
        const isOwnProduct = isOwner ? (product.branch_id === null) : (product.branch_id === userProfile.branch_id);
        ...
        try {
            ...
            if (modalMode === "create") {
                const prodPayload = {
                    ...
                    branch_id: isOwner ? null : userProfile.branch_id,
                ...
                const prodRes = await fetch("/api/backend/products", {
                    method: "POST",
                ...
                if (!prodRes.ok) {
                    ...
                    alert(errData.error || "Gagal membuat produk.");
                    return;
                }

                const prodJson = await prodRes.json();
                const newProductId = prodJson.data.id;
                ...
                if (branchStocksArray.length > 0) {
                    await fetch("/api/backend/stocks", {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                ...
                const branchId = userProfile?.branch_id;
                const stockRes = await fetch("/api/backend/stocks", {
                    method: "PATCH",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                        product_id: selectedProduct?.id,
                ...
                if (!stockRes.ok) {
                    ...
                    alert(errData.error || "Gagal memperbarui stok cabang");
                    return;
                }
            }
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            setActionLoading(true);
            const res = await fetch(`/api/backend/products?id=${id}`, {
                method: "DELETE"
            });
            if (res.ok) {
                fetchUserAndData();
                                                    ...
                                                    {isOwnProduct ? (
                                                        <>
                                                            <button 
                                                                ...
                                                                className="p-2 hover:bg-zinc-100 text-zinc-600 rounded-xl transition-all"
                    ...
                    <div className="bg-white w-full max-w-2xl rounded-2xl border border-zinc-150 overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                        ...
                        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
                            {modalMode !== "adjust_stock" ? (
                                ...
                                /* Hanya Tampilan Input Stok Cabang untuk Staff (Read-Only Info Produk) */
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="p-4 bg-zinc-50 rounded-xl border border-zinc-150 grid grid-cols-2 gap-4">
                                            <div className="space-y-1.5">
                                                <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider block">Stok Fisik Saat Ini (pcs)</label>
                                                <input 
                                                    type="number" 
                                                    value={branchStocksInput[userProfile?.branch_id]?.stock ?? ""}
                                                    onChange={(e) => setBranchStocksInput(prev => ({
                                                        ...prev,
                                                        [userProfile?.branch_id]: {
                                                            ...prev[userProfile?.branch_id],
                                                            stock: e.target.value === "" ? "" : Number(e.target.value)
                                                        }
                                                    }))}
